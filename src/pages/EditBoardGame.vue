  ...
  <div class="container">
    ...
    >
      ...
      >
        <a-input-number
          disabled="true"
          v-model:value="formState.averageRating"
        />
      </a-form-item>
...
export default {
  ...
  methods: {
    ...
    async onFinish(values) {
      ...
      data.append(
        ...
        JSON.stringify({
          ...this.form,
          ...values,
          averageRating: this.formState.averageRating,
      ...
      try {
        ...
        const response = await request.put(`/boardgame/${gameId}`, data, {
          headers: {
        ...
        Swal.fire({
          ...
          text: "ไม่สามารถแก้ไขบอร์ดเกมได้",
          icon: "error",
          confirmButtonText: "ตกลง",
        });
    ...
    async fetchBoardGameDataById() {
      try {
        const gameId = this.$route.params.gameId;
        const response = await request.get(`/boardgame/${gameId}`);
        this.formState = response.data;
        const base64Data = `data:image/jpeg;base64,${this.formState.photoData}`;
        ...
        const fileName = this.formState.photoName;
